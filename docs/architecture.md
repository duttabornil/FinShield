...
FinShield is designed as a modular, responsive fraud defense console built for explainable behavioral anomaly scoring and transaction network analysis. The repository separates public benchmark-style ML data, imported account/transaction network data, and synthetic simulator scenarios.
...
The API currently preserves the original in-memory simulator store for compatibility. PostgreSQL schema creation and repeatable ingestion are available through `scripts/import_data.py`, but a PostgreSQL server was not available in the validation environment, so database-backed API reads were not claimed as complete.
